find_isolated <- function(world) {
  ...
  counter <- 0
  world$map$isolated <- sapply(1:nrow(world$map), function(i) {
    counter <<- counter + 1
    setTxtProgressBar(pb, counter)
    link <- neighbour_ids(world, i)
    neighbours <- world$map[i, ]
...
remove_isolated <- function(world, type = c("both", "land", "water")) {
  ...
  for (i in to_fix) {
    ...
    range_x <- wrapped_range(as.numeric(world$map$x[i]), 1, world$map_x)
    range_y <- wrapped_range(as.numeric(world$map$y[i]), 1, world$map_y)
